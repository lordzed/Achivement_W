...
│   └── 928960/                        # Godfall
│       ├── achievements.json          # Your achievement data (only one "Your achievement data" file is required)
│       ├── Epic.platform              # Platform indicator (displays the text before ".platform" on the game card below AppID)
│       ├── blacklist                  # Skipped achievements to display. (Useful in cases like with Godfall where Steam version has more achievements than Epic)
│       └── game-info.json             # Auto-generated by workflow (optional)
